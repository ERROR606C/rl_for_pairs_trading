...
def dqn(df_a, df_b, SEED = 12):
    ...
    sd_train = mean_std_window(pd.Series(spread[:split]), 10000).reshape(-1, 1)
    sd_test = mean_std_window(pd.Series(np.array(spread[split:])), 10000).reshape(-1, 1)
    ...
    # ── Hyper-parameters ──────────────────────────────────────────────────
    TRANSACTION_COST = 0.0001     # proportional cost per open/close
    INITIAL_CASH     = 100.0
    ...
    N_EPISODES       = 4000     # training episodes (each = full train series)

    EPS_START        = 1.00
    EPS_END          = 0.02
    EPS_DECAY        = 0.9995   # per-episode multiplicative decay
    # ─────────────────────────────────────────────────────────────────────
    ...
    class QNet(nn.Module):
        def __init__(self, state_dim: int, n_actions: int, hidden: int = 128):
            ...
            self.net = nn.Sequential(
                nn.Linear(state_dim, hidden), nn.Sigmoid(),
