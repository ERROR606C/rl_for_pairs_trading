...
class PairsTradingEnv:
    ...
    def state(self) -> np.ndarray:
        ...
        return np.array(
            [
                float(self.position),
                (self.spreads[idx]-self._roll_mean10[idx])/self._roll_mean10[idx],
                ...
                (self.spreads[idx]-self._roll_mean1000[idx])/self._roll_mean1000[idx],
                (self.spreads[idx]-self._roll_mean10000[idx])/self._roll_std10000[idx],
                (self.spreads[idx]-self._roll_mean40000[idx])/self._roll_mean40000[idx],
    ...
    def _open_position(
        ...
        self.n_units      = self._portfolio_value() / (pa + self.beta * pb)
