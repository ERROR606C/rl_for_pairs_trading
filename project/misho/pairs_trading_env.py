...
class PairsTradingEnv:
    ...
    STATE_DIM  = 4     # length of the observation vector
    ...
    def __init__(
        ...
        tc                Transaction cost as fraction of notional per trade leg
                          (deducted from realized PnL on every open and close).
        """
    ...
    def mean_std_window(self, window_size):
        s    = pd.Series(self.spreads)
        roll = s.rolling(window=window_size, min_periods=1)
        mean = roll.mean()
        std  = roll.std(ddof=1).fillna(1.0).replace(0.0, 1.0)
    ...
    def _precompute_spread_stats(self) -> None:
        ...
        self._roll_mean20, self._roll_std20 = self.mean_std_window(20)
    ...
    def step(
        ...
        isChange = ((action-1)!=self.position)
        ...
        if self.position == 0:
            ...
            # ACTION_HOLD, ACTION_LONG → no-op

        else:  # self.position == -1
            # Short: the only valid close action is ACTION_LONG
            if action == self.ACTION_LONG:
                self._close_position(s)
            # ACTION_HOLD, ACTION_SHORT → no-op
        ...
        # ---- Reward = ΔV ------------------------------------------- #
        reward = self._portfolio_value() - prev_value - isChange*self.operation_penalty
    ...
    def state(self) -> np.ndarray:
        ...
        return np.array(
            [
                ...
                (self.spreads[idx]-self._roll_mean20[idx])/self._roll_std20[idx],
