...
class PairTradingEnv(gym.Env):
    """
    Pairs trading environment — velocity + rolling half-life observation.

    Observation (5 floats):
        z_score   – current spread z-score (rolling past_window mean/std)
        ...
        hl_norm   – rolling half-life normalised to [0, 1]
                    (HL clipped to [1, 252] days then / 252;
                     small → fast reversion, large → slow/broken reversion)
    ...
    def __init__(
        ...
        self._min_t = past_window + 3   # need z[t-3] for velocity
    ...
    def _obs(self) -> np.ndarray:
        ...
        return np.array(
            [self._z[t], float(self._pos), z_vel1, z_vel3, self._hl_norm[t]],
            dtype=np.float32,
        )
